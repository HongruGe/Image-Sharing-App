...
module.exports = function(app, connection, bcrypt, saltRounds) {
    ...
    app.post('/register', function(req, res) {
        ...
        var confirmPassword = req.body['confirm-password']; // Retrieve "confirm password" from the form
        var email = req.body.email;
        var firstname = "";
        var lastname = "";

        if (!validateEmail(email)) {
            return res.render('pages/register', { title: 'Register', message: 'Invalid email address format. Please try again.' });
        }

        // Validate if the password and confirm password match
        if (password !== confirmPassword) {
            // Show password mismatch error message
            return res.render('pages/register', { title: 'Register', message: 'Passwords do not match. Please try again.' });
        }
        ...
        bcrypt.hash(password, saltRounds, function (err, hash) {
            ...
            connection.query(sql, function(err, results) {
                ...
                if (err) {
                    return res.render('pages/register', { title: 'Register', message: 'Username or Email duplicated. Please try again.' });
                } else {
                    var dataToEJS = {
                        title: 'Register',
                    ...
                    return res.render('pages/login', dataToEJS);
                }
            });
        });
    });
    ...
    app.post('/contact', function(req, res) {
        ...
        if (!validateEmail(email)) {
            return res.render('pages/contact', { title: 'Contact', message: 'Invalid email address format. Please try again.' });
        }
